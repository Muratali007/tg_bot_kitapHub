...
async def start_command(message: types.Message):
    await message.answer(
        "👋 Добро пожаловать в KitapHub!\n\n"
        "Выберите одну из функций:\n"
        "1️⃣ /ai_recommend - Рекомендации с помощью ИИ\n"
        "2️⃣ /kitaphub_recommendation - Рекомендации от KitapHub"
    )
...
async def kitap_hub_login(message: types.Message, state: FSMContext):
    ...
    if validate_login(username):
        await message.answer("Логин успешен! 🎉")
        await message.answer("книги генерируются, это займет 2-3 минуты 🔃")
        ...
        if book_ids:
            ...
            await message.answer("📚 Вот ваши рекомендации книг от KitapHub:")
            await message.answer(translated_titles)
        else:
            await message.answer("❌ Книги не найдены для вашего аккаунта.")
        ...
        await message.answer("Неправильный логин. Попробуйте еще раз. 🔄")
        await Form.kitaphub_login.set()
...
def recommend_books(ratings_mat, interactions, my_index, file_book_titles):
    ...
    top_10_titles = book_recs["title"].head(5).tolist()
